...
const logger = rootLogger.child({ journey: 'request', route: '/api/recommendations/[id]/dismiss' });
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    logger.info({ recommendationId: id }, 'Dismissed recommendation');
    ...
    logger.error({ err: error instanceof Error ? error : new Error(String(error)) }, 'Failed to dismiss recommendation');
